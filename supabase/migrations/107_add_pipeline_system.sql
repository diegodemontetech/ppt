...
CREATE TABLE pipeline_fields (
    ...
    name TEXT NOT NULL,
    label TEXT NOT NULL,
    type TEXT NOT NULL, -- text, textarea, number, date, select, multiselect, file, etc
    mask TEXT, -- Brazilian masks (CPF, CNPJ, phone, plate, etc)
...
BEGIN
    ...
    LOOP
        ...
        IF breach_hours > 0 THEN
            ...
            -- Execute any automation rules for SLA breach
    ...
    LOOP
        CASE automation.action_type
            ...
            WHEN 'assign_user' THEN
                UPDATE pipeline_cards
                SET assigned_to = (automation.action_config->>'user_id')::uuid
                WHERE id = card_id;
