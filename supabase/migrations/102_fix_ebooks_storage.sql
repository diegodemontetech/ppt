-- Fix e-books storage and access
BEGIN;

-- Create PDFs bucket if not exists
...
-- Update existing e-books to use correct file paths
UPDATE ebooks
SET pdf_url = CASE
  WHEN pdf_url NOT LIKE 'http%' THEN pdf_url
  ELSE split_part(pdf_url, '/', -1)
END;
